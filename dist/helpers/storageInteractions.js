export const getAndConvertLocalStorage = () => {
    let mistakesAdd = JSON.parse(localStorage.getItem("mcm_add")) || [];
    let mistakesSubstract = JSON.parse(localStorage.getItem("mcm_substract")) || [];
    let mistakesMultiply = JSON.parse(localStorage.getItem("mcm_multiply")) || [];
    let mistakesDevide = JSON.parse(localStorage.getItem("mcm_devide")) || [];
    return { mistakesAdd, mistakesSubstract, mistakesMultiply, mistakesDevide };
};
export const addWrongOperationToApplicationStorageAndLocalStorage = (wrongOperations, storage) => {
...
export const removeOperationsFromAppStorageAndLocalStorage = (removedValuesArr, applicationStorage) => {
    switch (removedValuesArr[2]) {
        case "add":
            let updatedMistakesAdd = applicationStorage.mistakesAdd.filter((el) => el.join(",") !== removedValuesArr.slice(0, 2).join(","));
            localStorage.setItem("mcm_add", JSON.stringify(updatedMistakesAdd));
            applicationStorage = getAndConvertLocalStorage();
            break;
        case "devide":
            let updatedMistakesDevide = applicationStorage.mistakesDevide.filter((el) => el.join(",") !== removedValuesArr.slice(0, 2).join(","));
            localStorage.setItem("mcm_devide", JSON.stringify(updatedMistakesDevide));
            applicationStorage = getAndConvertLocalStorage();
            break;
        case "substract":
            let updatedMistakesSubstract = applicationStorage.mistakesSubstract.filter((el) => el.join(",") !== removedValuesArr.slice(0, 2).join(","));
            localStorage.setItem("mcm_substract", JSON.stringify(updatedMistakesSubstract));
            applicationStorage = getAndConvertLocalStorage();
            break;
        case "multiply":
            let updatedMistakesMultiply = applicationStorage.mistakesMultiply.filter((el) => el.join(",") !== removedValuesArr.slice(0, 2).join(","));
