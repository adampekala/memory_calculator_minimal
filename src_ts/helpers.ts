...
export const updateConvertedStorage = (
  ...
  switch (removedValuesArr[2]) {
    ...
    case "devide":
      ...
      break;
    case "substract":
      let updatedMistakesSubstract = storage.mistakesSubstract.filter(
        (el) => el.toString() !== removedValuesArr.toString()
      );
      localStorage.setItem(
        "mcm_substract",
      ...
      break;
    case "multiply":
      let updatedMistakesMultiply = storage.mistakesMultiply.filter(
        (el) => el.toString() !== removedValuesArr.toString()
      );
      localStorage.setItem(
        "mcm_multiply",
